...
let cache: { data: TransferData[]; timestamp: number } | null = null;

/** Pending promise for request deduplication. */
let pendingPromise: Promise<TransferData[]> | null = null;
...
async function loadData(): Promise<TransferData[]> {
  // Deduplicate: if a request is already in-flight, reuse it
  if (pendingPromise) return pendingPromise;

  pendingPromise = fetchTransfers()
    .then((data) => {
...
export function useTransfers(): UseTransfersReturn {
  const [transfers, setTransfers] = useState<TransferData[]>(cache?.data ?? []);
  ...
  const mutate = useCallback((data: TransferData[]) => {
    cache = { data, timestamp: Date.now() };
    notifySubscribers();
  }, []);

  return { transfers, isLoading, error, refetch, mutate };
}

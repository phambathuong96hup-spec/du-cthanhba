...
function getChartColors() {
    ...
    return { textColor: isDark ? '#e2e8f0' : '#334155', bg: 'transparent' };
}
...
function drawCharts(tasks, compliance) {
    ...
    const pieData = google.visualization.arrayToDataTable([
        ['Trạng thái', 'SL'], ['Đã xong', stats.Done], ['Đang làm', stats.Doing], ['Quá hạn', stats.Overdue]
    ]);
    new google.visualization.PieChart(document.getElementById('piechart_task')).draw(pieData, {
        colors: ['#10b981', '#3b82f6', '#ef4444'], pieHole: 0.55, fontName: chartFont,
        legend: { position: 'bottom', textStyle: { color: textColor, fontSize: 11 } },
        chartArea: { width: '90%', height: '75%' }, backgroundColor: bg
    });

    // Pie chart — Difficulty
    const diffData = google.visualization.arrayToDataTable([
        ['Độ khó', 'SL'], ['Dễ', diffStats['1']], ['TB', diffStats['2']], ['Khó', diffStats['3']], ['Phức tạp', diffStats['4']]
    ]);
    new google.visualization.PieChart(document.getElementById('piechart_difficulty')).draw(diffData, {
        colors: ['#34d399', '#fbbf24', '#f87171', '#a78bfa'], pieHole: 0.55, fontName: chartFont,
        legend: { position: 'bottom', textStyle: { color: textColor, fontSize: 11 } },
        chartArea: { width: '90%', height: '75%' }, backgroundColor: bg
    });
    ...
    if (arr.length > 1) {
        new google.visualization.ColumnChart(document.getElementById('barchart_task')).draw(
            ...
            isStacked: true, colors: ['#10b981', '#3b82f6', '#ef4444'], fontName: chartFont,
            legend: { position: 'bottom', textStyle: { color: textColor, fontSize: 11 } },
            chartArea: { width: '85%', height: '70%' }, backgroundColor: bg,
...
function drawComplianceChart(data, textColor, font, bg) {
    ...
    Object.keys(rewards).sort().forEach(k => { if (rewards[k] > 0) rArr.push([k, rewards[k], '#f59e0b']); });
...
function renderCal(tasks, compliances, today) {
    ...
    tasks.forEach(t => {
        ...
        if (dateStr) {
            ...
            events.push({
                title: (isDone ? '✅ ' : '⏰ ') + t[1], start: dateStr,
    ...
    compliances.forEach(r => {
        if (r[1]) {
            ...
            events.push({
                title: (isReward ? '🏆 ' : '⚠️ ') + r[2], start: r[1],
                backgroundColor: isReward ? '#f59e0b' : '#ef4444',
                borderColor: isReward ? '#f59e0b' : '#ef4444', allDay: true,
